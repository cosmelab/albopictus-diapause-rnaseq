...
#   VALIDATE_ONLY=1 sbatch scripts/05_rnaseq_pipeline/02_run_rnaseq.sh
#
...
NXF_CMD=(
    ...
    -name "${RUN_NAME}"
    -profile singularity
    -c scripts/05_rnaseq_pipeline/hpc_batch.conf
    -params-file scripts/05_rnaseq_pipeline/params.yaml
    -work-dir "${NXF_WORK}"
    -resume
...
if [ "${VALIDATE_ONLY}" = "1" ]; then
    ...
    if [ ! -f scripts/05_rnaseq_pipeline/params.yaml ]; then
        echo "❌ ERROR: Parameters file not found: scripts/05_rnaseq_pipeline/params.yaml"
        exit 1
    fi
    ...
    echo "  sbatch scripts/05_rnaseq_pipeline/02_run_rnaseq.sh"
    exit 0
fi
