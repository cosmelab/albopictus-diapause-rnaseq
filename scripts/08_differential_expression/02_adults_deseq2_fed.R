#!/usr/bin/env Rscript
################################################################################
# Adult Female Differential Expression Analysis - Blood-Fed Only
#
...
#
# This script analyzes BLOOD-FED females only:
# - LD_Fed (16L:8D) vs SD_Fed (8L:16D, diapause)
# - Design: ~ Photoperiod
...
cat("================================================================================\n")
cat("Adult Differential Expression Analysis - Blood-Fed Females\n")
cat("================================================================================\n\n")
...
# Subset to fed samples only
cat("\nFiltering for BLOOD-FED samples only...\n")
fed_samples <- metadata[grepl("Fed", metadata$New.abreviation) & !grepl("Unfed", metadata$New.abreviation), ]
cat("  Fed samples:", nrow(fed_samples), "\n")
cat("  Groups:\n")
print(table(fed_samples$New.abreviation))

# Subset counts to fed samples
counts_fed <- counts[, rownames(fed_samples)]
cat("\n  Count matrix subset:", nrow(counts_fed), "genes x", ncol(counts_fed), "samples\n")

# Create simplified photoperiod column
fed_samples$Photoperiod <- ifelse(grepl("LD", fed_samples$New.abreviation), "LD", "SD")
fed_samples$Photoperiod <- factor(fed_samples$Photoperiod, levels = c("LD", "SD"))

cat("\n  Photoperiod groups:\n")
print(table(fed_samples$Photoperiod))
...
cat("  Original genes:", nrow(counts_fed), "\n")

# Filter: keep genes with at least 10 counts in at least 25% of samples
min_samples <- ceiling(0.25 * ncol(counts_fed))
keep <- rowSums(counts_fed >= 10) >= min_samples
counts_filtered <- counts_fed[keep, ]

cat("  After filtering:", nrow(counts_filtered), "genes\n")
cat("  Removed:", nrow(counts_fed) - nrow(counts_filtered), "genes\n")
cat("  Retention rate:", round(100 * nrow(counts_filtered) / nrow(counts_fed), 2), "%\n")
...
cat("  Contrast: SD (diapause) vs LD (non-diapause)\n\n")
...
# Save all results
all_results_file <- file.path(output_dir, "adults_fed_deseq2_all_LFCshrink.tsv")
...
sig_results_file <- file.path(output_dir, "adults_fed_deseq2_significant_LFCshrink.tsv")
...
summary_file <- file.path(output_dir, "adults_fed_summary_stats.tsv")
...
pdf(file.path(output_dir, "adults_fed_pca.pdf"), width = 8, height = 6)
plotPCA(vsd, intgroup = "Photoperiod") +
  ggtitle("PCA - Adult Females (Blood-Fed Only)") +
  theme_bw() +
...
pdf(file.path(output_dir, "adults_fed_correlation_heatmap.pdf"), width = 10, height = 10)
pheatmap(vsd_cor,
         main = "Sample Correlation - Adult Females (Blood-Fed)",
         fontsize = 10)
dev.off()

# MA plot (both unshrunk and shrunk)
cat("  Creating MA plots...\n")
pdf(file.path(output_dir, "adults_fed_ma_plot.pdf"), width = 12, height = 6)
par(mfrow = c(1, 2))
plotMA(res, main = "MA Plot - Unshrunk\nAdult Females (Blood-Fed) SD vs LD", ylim = c(-5, 5))
...
pdf(file.path(output_dir, "adults_fed_dispersion.pdf"), width = 8, height = 6)
plotDispEsts(dds, main = "Dispersion Estimates - Adult Females (Blood-Fed)")
dev.off()
...
cat("\n\n================================================================================\n")
cat("ANALYSIS COMPLETE - Blood-Fed Females\n")
cat("================================================================================\n\n")
...
# Save session info
sink(file.path(output_dir, "adults_fed_session_info.txt"))
cat("Adult Blood-Fed DESeq2 Analysis\n")
